...
uint16_t sensor_wakeup_time=290;
uint16_t sensor_active_time=10;


uint32_t dyn_sen_rd_time_interval=0;  

uint16_t adv_interval=800;
uint32_t power_transmission=0;
uint32_t volatile epoch_time=1506064608;
uint8_t hr_led_int_config[2]={0x09,0xFF};
uint8_t device_name[]="Embdes_Test";                              


bool read_time_interval_receive=false;
bool adv_interval_receive=false;
bool current_adv_rd_time_response=false;
bool data_ready=false;
bool timer_stop=false;
extern bool data_tx_flag;
extern bool data_tx_ack_flag;
extern bool sensor_data_rx_flag;
extern uint16_t  m_sample_write_idx;
extern uint16_t m_sample_read_idx;
extern bool no_data_flag;
extern bool sensor_wakeup_time_update_flag;
		
extern uint8_t epoch_time_ack;
extern bool epoch_time_receive;
extern bool sensor_data_adv_time_request;
extern bool read_time_interval_receive;
extern bool adv_interval_receive;
extern bool timer_stop;
extern bool device_name_updated;
...
/*
   user can dynamically configurable parameter types
*/
#define EPOCH_TIME_TYPE                   0xA0
#define	SENSOR_DATA_ADV_TIME_TYPE         0xA2 
#define	SENSOR_WAKEUP_ADV_TIME_TYPE       0xA4 
#define SENSOR_DATA_TRANSFER_TYPE         0X0D
#define DEVICE_NAME_UPDATE_TYPE           'E'           //0x0E


bool hrs_notification_enabled = false;
        ...
        if (p_hrs->evt_handler != NULL)
        {
            ...
            if (ble_srv_is_notification_enabled(p_evt_write->data))
            {
                evt.evt_type = BLE_HRS_EVT_NOTIFICATION_ENABLED;
							  hrs_notification_enabled = true;
            }
            else
            {
                evt.evt_type = BLE_HRS_EVT_NOTIFICATION_DISABLED;
							  hrs_notification_enabled = false;
            }
...
/**@brief Function for Update the Timer ticks.
 *
 * @return Update Timrer ticks 
 */
//uint32_t update_time_timer_ticks()
//{
//	return ((uint32_t)((sensor_wakeup_time_interval*(uint64_t)APP_TIMER_CLOCK_FREQ)+((((APP_TIMER_PRESCALER) + 1) * 1000)/2))/(((APP_TIMER_PRESCALER) + 1) * 1000));
//}
...
static void on_write(ble_hrs_t * p_hrs, ble_evt_t * p_ble_evt)
{
    ...
    if (p_evt_write->handle == p_hrs->hrm_handles.cccd_handle)
    {
        on_hrm_cccd_write(p_hrs, p_evt_write);
			...
			if(p_evt_write->handle==0x000E)	
		   {
		
    		  switch(p_evt_write->data[0])
		     	{
		
			        case EPOCH_TIME_TYPE:
					         time=((uint16_t)p_evt_write->data[2]<<8)|(p_evt_write->data[3]);
				 	         epoch_time=time;
				           time=((uint16_t)p_evt_write->data[4])<<8|(p_evt_write->data[5]);
			             epoch_time=(epoch_time<<16)|time;
				           epoch_time_receive=true;
							     break;
							case SENSOR_DATA_ADV_TIME_TYPE:
								   current_adv_rd_time_response=true;
							     break;
				
				      case SENSOR_WAKEUP_ADV_TIME_TYPE:
								
									 adv_interval=(((uint16_t)p_evt_write->data[2])<<8)|(p_evt_write->data[3]);
									 sensor_wakeup_time=p_evt_write->data[4];
									 sensor_wakeup_time=((sensor_wakeup_time*60)-10);
							
							     if((p_evt_write->data[5]>=5))
									 {	 
							       sensor_active_time=p_evt_write->data[5];
									 }
									 
									
      						 adv_interval_receive=true;
		               read_time_interval_receive=true;
							     sensor_wakeup_time_update_flag=true;
									 timer_stop=true;
							     break;
							
							case DEVICE_NAME_UPDATE_TYPE:
								  
								   
							     memset(device_name,0,sizeof(device_name));
							
							     p_evt_write->data[10]=NULL;
								   
							     for(i=0;p_evt_write->data[i]!=NULL;i++)
						         {
										     device_name[i]=p_evt_write->data[i+1]; 
										 } 
								   device_name_updated=true;											 
							     break;
										 
							case SENSOR_DATA_TRANSFER_TYPE:
								   if(m_sample_write_idx==m_sample_read_idx)
									 {
									    no_data_flag=true;
									 }
									 else
									 {		 
								      data_tx_flag=true;
							        sensor_data_rx_flag=false;
							        data_tx_ack_flag=true;
									 }
									 break;
...
uint32_t ble_hrs_heart_rate_measurement_send(ble_hrs_t * p_hrs, void const *heart_rate,uint16_t no_samples)
{
    uint32_t err_code;
